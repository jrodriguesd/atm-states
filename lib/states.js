    ...
    case 'K':
      {
        ...
        // state_to is used bu State Navigator and must be Set
        let states_to = new Set();
        for(let i = 2; i < 10; i += 1)
          states_to.add(this.getEntry(data, i));
        parsed.set('states_to', states_to);

        // state_exits is used by state processing, and must be an array
        let state_exits = [];
        for(let i = 2; i < 10; i += 1)
          state_exits.push(this.getEntry(data, i));
        parsed.set('state_exits', state_exits);
      }
      break;
